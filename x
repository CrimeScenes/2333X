...
local function UpdateFOV()
    ...
    local success, errorMsg = pcall(function()
        if Circle then
            Circle.Visible = Forbidden.PointLock.Radius_Visibility
            Circle.Radius = Forbidden.PointLock.Radius
        ...
        if BodyPart then
            ...
            if Forbidden.PointLock.Aligner then
                ...
                if humanoid then
                    ...
                    predictedPosition = BodyPart.Position + (moveDirection * Forbidden.PointLock.Prediction)
                end
            else
                local targetVelocity = TargetPlayer.Character.HumanoidRootPart.Velocity
                predictedPosition = BodyPart.Position + (targetVelocity * Forbidden.PointLock.Prediction)
            end
